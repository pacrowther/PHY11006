...
The work done during an ospthermal process can be calculated by using $P = nRT/V$. Assuming a fixed mass of gas, $n$, $R$ and $T$ are all 
constant, so
...
But since $P V^{\gamma}$ is constant, $K = P_{f} V_{f}$ we can simpify this to
