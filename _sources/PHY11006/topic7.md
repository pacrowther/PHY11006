...
For the Clausus and Kelvin statements, we can prove both implications by contradiction.

## Clausius implies Kelvin

Suppose that Claus' statement is true but Kelvin's is not. If Kelvin's statment is not true we can build an engine K that converts heat into 
...
of taking heat $Q_C$ from the cold reservior and transferring it to the hot reservior. But this is in violation of Clausius' statement, 
which contradicts our initial assunption. Therefore, Clausuis' statment implies Kelvin's statement.

## Kelvin implies Clausius

We can use the same approach here. Assume that Clausius' statement is not true, and that we can therefore build a machnie that simply 
transports heat $Q_{C}^{'}$ from a cold reservior to a hot reservior. Connect this to a Carnot engine running forwards, which takes heat 
$Q_{H}$ from a hot reservior, does work $W$, and rejects heat $Q_{C}$. If we adjust the Clausius machine so that $Q_{C} = Q_{C}^{'}$, then 
...
Kelvin's tatement is true, Clausius' statement must also be true.
...
Assume that Carnot's statement is not true, and we can therfore build an engine $E$ that has a higher efficiency than a Carnot engine. We 
...
$Q_{C} - Q_{C}^{'}$ from the cold reservior and eject it into the hot reservior, which violates Clausius' statement, Therefore Clausius' 
statement of the Second Law implies that engine $E$ cannot exist.

# The Third Law of Thermodynamics

If we could run a Carnot engine with itsw cold reservior at 0 K, we could violate Kelvin's statement, because such an engine would have 
$\eta$ = 1, wnd would therefore convert all the input heat into work. This is forbidden by the __Third Law of Thermodynamics__, Nernst's 
statement of which is

<div class="alert alert-block alert-info">
It is impossible for any procedure to lead to the isotherm T = 0 in a finte number of steps.
</div>
...
We can generalise this sum to an integral around a clused loop (imagine lots of mini Carnot cycles working between temperatures $T$ and 
...
:align: center
:width: 50%
Two arbritary paths between states A and B
```

To see this, consider states A and B in Fig 7.3. The two paths connecting a and B are both reversible. We can integrate from A to B along 
path 1 and get $\Delta S_{1} = \int_{1} \frac{\bar{d}Q}{T}$, or we can integrate along path 2 and get $\Delta S_{2} = \int_{2} 
\frac{\bar{d}Q}{T}$. But we can also reverse the direction of path 2, since we have defined it to be reversible, and intergate from B to A, 
...
This definition only allows us to calculate _changes_ in entropy, $\Delta S$; if we try to cancluate the absolute value of $S$ we have an 
undetermined integration constant. To calculate absolute values of $S$ we need a microscopic defintion in terms of individual particles. 
you will meet this defintion (and thow that it is equivalent to the definition in Eqn {eq}`eqn7.2`) when you study statistical mechanics 
next 
year.
...
dU = T dS - P dV
```
But now a miracle has occured: _everything_ in Eqn {eq}`eqn7.4` is a function of state, and therefore Eqn {eq}`eqn7.4` holds 
...
process.  (This doesn't mwean that the heat supplied and the work done will be the same: $\bar{d}Q$ and $\bar{d}W$ are still inexact 
...
Irreversible processes are irreversible because energy has dissipated in a way that cannoout be reversed .e.g friction. This is clearly not 
...
$$
(again, recvall that the minus sign here is because the woirk is done _by_, not _on_, the system). So we have
...
defintion $dS = \bar{d}Q/T$ does not apply. This means that if we wish to calculate the entropy change associated with an irreversible 
process,m we cannot simply integrate $\bar{d}Q/T$. Fortunately, there is a solution to this, because entropy is a function of state: if we 
can define a _reversible_ path with the same initial and final stete, we can use this to calculate $\Delta S$.
...
equilibrium at temperature $T_{R}$ (we assume that the heat transfer to or from the samll system has negligible effect on the temperature of 
...
volume of the small system does not change, and that the combined system of the small systtem plus the reservoir is thermally isolated from 
...
the Second Law. However, we can replace it with a reversible transfer of heat at constant temperature for the reservior, and constant volume 
...
small system) and newgative if the small system was initially warmer (heat is lost _from_ the small system). Since the combined system is 
...
1. Although the temperature of the reservior does not change, its entropy does.
...
In the above example, the entropy chanfge of the small system is negative when the system cools to equilibrium, and positive when it warms, 
...
positive (except in the case where the two systems were already in equilibrium before being brought into contact). In constrast, if we have 
...
entropy of the system may indeed decrease, but it is not a thermally isolatesd system, and the decrease is compensated by an increase in the 
entropy of the surroundings, as in Example 7.1

The Universe is the ultimate thermally isolated system, so the entropy statement of the Second Law implies that the entropy of the Univerrse 
cannot decrease.
...
We can do the adiabatic steps 2 and 4 immediatel: by definition, in an adiabatic process no heat is transfered, so $\Delta S_{\rm sys} = 
\Delta S_{\rm surr} = 0$.
...
an itegration because the temperature is constant). Converrsely, the surroundings (the cold reservoir) _receive_ heat $|Q_{C}|$, so the 
...
function of state; (2) the entropy change of _system plus surroundings_ is zero not not for the cycle as a whole, but also for each 
...
dissipation of energy - in esssence , some of the energy that would, in a reversible system, be released as work (ordered motion) is instead 
