...
temperature and density in this region are $1.56 \times 10^7$ K and $1.48 \times 10^5$  kg/m$^3$. The radius ofthe proton is about 
1 fm, and the radius of a helium nucleus is about 1.7 fm; the classical electron raius is about 2.8 fm. Is it reasonable to treat the 
...
every helium. If they are all fully ionized, that means that the particles in the Solar core are in the ratio 4 electons: 2 protons: 1 
...
means that there are $1.0 x 10^{32}$ particles per cubmic metere, so each particle occupies a volume of $ 1.0 \times 10^{-32}$ m$^3$. 
...
electron is $< 1.0 \times 10^{-43}$ m$^3$. Therefore the volume occupied by the particles is surely negligible compared to the volme 
occupied by the gas.

We might, however, worry about the assumption that the particles do not exert any force on each other - as they are all charged, turely this 
...
(this conclusion was first reached by Sir Arthur Standley Eddington in the 1920s, and resulted in the development of modern theories of 
stellar interiors.)
``` 
````
`````
...
where $R = N_A k_B$ = 8.3145 J/mol/K is the gas constant and $N_A = 6.022 \times 10^{23}$ /mol is Avogadro's
number.
...
:align: center
:width: 50%
Isotherms, or fixed-temperature curves, for a constant abount of an ideal gas. Source: Fig 18.6 from University Physics with Modern Physics 
(Pearson)
```
...
volume, 
the heat energy supplied cannout do anything except increase the internal energy of the gas. This would not be true if we did not keep the 
...
three components of the particle velocity as in Equation {eq}`eqn3.6`. This will be true of nomatomic gases like helium and neon. It is 
_not_ true of 
...
We therefore conclude that the total number of degrees of freedmon is 3 + 2 + 2 = 7, and so raising the temperature of a mole of such a gas 
...
:align: center
Molar heat capacities of some diatomic gases, as a funciton of temperature. Data from [NIST-JANAF thermochemical 
...
noble gases) behaves as  we expect,  but the  diatmoic gases approach $7/2 R$ only at high temperatures. At room temperature common diatomic 
...
:align: center
Experimental values of $C_{V}$ for lead, aluminium, solicon and diamond. Source: Fig 18.21 from University Physics with Modern Physics 
```
...
rotational and vibrational degrees of freedmon are active in this molecule?
```
...
In reality, gas molecules have finite sizes, so may collide with one another. We can extimate the frequency of collisions and distance 
dravelled between collisions, by considering $N$ spherical, identical molecules of radius $r$ in a volume $V$. Considering the motion of a 
...
$4 \pi r^{2} v dt.$ There are $N/V $molcules per unit volume so the number $dN$ with centres in the cylinder are $dN = 4 \pi r^{2} v dt 
...
```
The more molecules there are and the larger the molecule, the shorter the distance betqween collisions. Note that the mean free path does 
...
increases. If the pressure is increase at fixed temperature, the gas compreses and $\lambda$ decreases.
...
C  using $v = v_{\rm rms}$ from Example 3.3. We obtain $\lambda= 5.6 \times 10^{-8}$ m and $t_{\rm mean} = 1.2 \times 10^{-10}$ s. A typical 
